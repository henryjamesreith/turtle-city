...
function ApartmentRoom({ hasSkateboard, nearDoor, onNearDoor, onExit, turtleName, turtleVariant, upgrades }: { hasSkateboard: boolean; nearDoor: boolean; onNearDoor: (near: boolean) => void; onExit: () => void; turtleName: string; turtleVariant: TurtleVariant; upgrades: string[] }) {
  const freshWalls = upgrades.includes("apartment-fresh-walls");
  const warmLights = upgrades.includes("apartment-warm-lights");
  const comfyBed = upgrades.includes("apartment-comfy-bed");
  return <>
    ...
    <mesh position={[0, 4, -6.4]} receiveShadow><boxGeometry args={[18, 8, 0.28]} /><meshStandardMaterial color={freshWalls ? "#e8dfcb" : "#c6b28e"} roughness={0.94} /></mesh>
...
export function ChelseaApartment3D({ hasSkateboard, onExitToChelsea, onPurchaseUpgrade, shells, turtleName, turtleVariant, upgrades }: { hasSkateboard: boolean; onExitToChelsea: () => void; onPurchaseUpgrade: (itemKey: string) => Promise<void>; shells: number; turtleName: string; turtleVariant: TurtleVariant; upgrades: string[] }) {
  const [nearDoor, setNearDoor] = useState(false);
  const [storeOpen, setStoreOpen] = useState(false); const [buying, setBuying] = useState(""); const [error, setError] = useState("");
  ...
  return <main className="interior3d-stage" data-testid="chelsea-apartment-3d"><Canvas camera={{fov:48,near:.1,far:80,position:[8,9,14]}} dpr={[1,1.5]} shadows="basic"><Suspense fallback={null}><ApartmentRoom hasSkateboard={hasSkateboard} nearDoor={nearDoor} onNearDoor={setNearDoor} onExit={onExitToChelsea} turtleName={turtleName} turtleVariant={turtleVariant} upgrades={upgrades} /></Suspense></Canvas><header className="interior3d-title"><p>Chelsea · Apartment 4B</p><h1>Your apartment</h1><span>{upgrades.length ? `Tier ${upgrades.length} · ${upgrades.length} improvements` : "Starter condition · needs work"}</span></header><button type="button" className="apartment-store-button" onClick={() => setStoreOpen(true)}>🐚 {shells} · Improve apartment</button><aside className="interior3d-controls">WASD to move{hasSkateboard ? " · R to ride / walk" : ""} · Enter near the door</aside>{nearDoor ? <aside className="interior3d-prompt"><div><strong>Apartment 4B</strong><small>Go outside to Chelsea.</small></div><button type="button" onClick={onExitToChelsea}>Go outside</button></aside> : null}{storeOpen ? <section className="apartment-store"><button type="button" className="store-close" onClick={() => setStoreOpen(false)}>×</button><p>Apartment 4B</p><h2>Improvements</h2><strong>🐚 {shells} Shells</strong>{items.map((item) => { const owned = upgrades.includes(item.key); return <div key={item.key}><span><b>{item.name}</b><small>{owned ? "Installed" : `${item.price} Shells`}</small></span><button type="button" disabled={owned || buying !== "" || shells < item.price} onClick={async () => { setBuying(item.key); setError(""); try { await onPurchaseUpgrade(item.key); } catch (purchaseError) { setError(purchaseError instanceof Error ? purchaseError.message : "Purchase failed."); } finally { setBuying(""); } }}>{owned ? "Owned" : buying === item.key ? "Installing…" : "Buy"}</button></div>; })}{error ? <em>{error}</em> : null}</section> : null}<style jsx>{`.apartment-store-button{position:absolute;top:28px;right:30px;padding:12px 16px;border:0;border-radius:12px;background:#fff0b0;color:#244636;font-weight:900;cursor:pointer}.apartment-store{position:absolute;z-index:8;top:50%;right:30px;width:320px;transform:translateY(-50%);padding:24px;color:#17382b;background:#fff8df;border-radius:18px;box-shadow:0 20px 60px #18382b66;font-family:system-ui}.apartment-store>p{margin:0;text-transform:uppercase;font-weight:850;letter-spacing:.1em}.apartment-store h2{margin:5px 0}.apartment-store>strong{display:block;margin-bottom:14px}.apartment-store>div{display:flex;align-items:center;gap:12px;padding:12px 0;border-top:1px solid #d9ccb2}.apartment-store span{display:grid;flex:1}.apartment-store small{opacity:.68}.apartment-store button{padding:9px 12px;border:0;border-radius:8px;background:#285942;color:#fff;font-weight:800}.apartment-store button:disabled{opacity:.5}.store-close{position:absolute;right:14px;top:14px}.apartment-store em{display:block;color:#a13f31;font-weight:700}`}</style><InteriorStyles /></main>;
}
