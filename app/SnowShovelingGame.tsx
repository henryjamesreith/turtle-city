...
type ShiftMode = "timed" | "relaxed";
type SnowBank = { x: number; y: number; amount: number; seed: number };
type ShiftState = { x: number; y: number; facingX: number; facingY: number; timeLeft: number; mode: ShiftMode; status: ShiftStatus; cleared: Set<number>; shovelDown: boolean; shovelLoad: number; banks: SnowBank[]; startedAt: number; message: string; messageUntil: number; stride: number };
...
type ShiftHud = { status: ShiftStatus; timeLeft: number; mode: ShiftMode; clearedPercent: number; shovelLoad: number; message: string };
...
function createShiftState(status: ShiftStatus = "ready", mode: ShiftMode = "timed"): ShiftState { return { x: 600, y: 625, facingX: 0, facingY: -1, timeLeft: SHIFT_LENGTH, mode, status, cleared: new Set(), shovelDown: false, shovelLoad: 0, banks: [], startedAt: performance.now(), message: "", messageUntil: 0, stride: 0 }; }
...
function SnowWorld({ view, turtleVariant }: { view: ShiftView; turtleVariant: TurtleVariant }) {
...
export function SnowShovelingGame({ onExit, turtleName, turtleVariant }: SnowShovelingGameProps) {
  ...
  const [hud, setHud] = useState<ShiftHud>({ status: "ready", timeLeft: SHIFT_LENGTH, mode: "timed", clearedPercent: 0, shovelLoad: 0, message: "" });
  function startShift(mode: ShiftMode = hud.mode) { const next = createShiftState("playing", mode); gameRef.current = next; setView(createShiftView(next)); setHud({ status: "playing", timeLeft: SHIFT_LENGTH, mode, clearedPercent: 0, shovelLoad: 0, message: "Hold Space and push forward through the snow" }); }
  useEffect(() => {
    ...
    function update(time: number) { const state = gameRef.current; const elapsed = Math.min((time - previousTime) / 1000, 0.05); previousTime = time;
      if (state.status === "playing") { const horizontal = Number(pressed.has("arrowright") || pressed.has("d")) - Number(pressed.has("arrowleft") || pressed.has("a")); const vertical = Number(pressed.has("arrowdown") || pressed.has("s")) - Number(pressed.has("arrowup") || pressed.has("w")); const magnitude = Math.hypot(horizontal, vertical);
        ...
        if (state.mode === "timed") state.timeLeft = Math.max(0, SHIFT_LENGTH - (time - state.startedAt) / 1000); const clearedPercent = state.cleared.size / pathCells.length * 100; if (clearedPercent >= CLEAR_TARGET || (state.mode === "timed" && state.timeLeft <= 0)) { state.status = "finished"; if (state.shovelLoad > 0) dumpShovel(state, time); }
        if (time - previousHudUpdate > 70 || state.status === "finished") { previousHudUpdate = time; setView(createShiftView(state)); setHud({ status: state.status, timeLeft: state.timeLeft, mode: state.mode, clearedPercent, shovelLoad: state.shovelLoad, message: time < state.messageUntil ? state.message : "" }); }
      }
  ...
  return <main className="shoveling-stage" data-testid="snow-shoveling-game"><div className="shoveling-canvas" aria-label={`3D Snow Crew shoveling yard for ${turtleName}`}><Canvas orthographic camera={{ position: [0, 14, 15], zoom: 46 }} dpr={[1, 1.5]} shadows="basic" gl={{ antialias: true, powerPreference: "high-performance" }}><Suspense fallback={null}><SnowWorld view={view} turtleVariant={turtleVariant} /></Suspense></Canvas></div><button type="button" className="shoveling-exit" onClick={onExit}><span aria-hidden="true">←</span>Central Park</button><section className="shoveling-scoreboard" aria-live="polite"><div><small>Route open</small><strong>{Math.min(100, Math.floor(hud.clearedPercent))}%</strong></div><span aria-hidden="true"><i style={{ width: `${Math.min(100, hud.clearedPercent)}%` }} /></span><time>{hud.mode === "relaxed" ? "No timer" : formatTime(hud.timeLeft)}</time></section>{hud.status === "playing" ? <><aside className="shovel-load" aria-live="polite"><small>Shovel load</small><span aria-hidden="true"><i style={{ width: `${hud.shovelLoad / MAX_LOAD * 100}%` }} /></span><strong>{hud.shovelLoad >= MAX_LOAD - 1 ? "FULL" : "PUSH"}</strong></aside><p className="shoveling-coach">{hud.message || "Hold Space to lower the blade · Release off the path to dump"}</p></> : null}{hud.status !== "playing" ? <section className="shoveling-start-card"><p>Central Park Snow Crew</p><h1>{hud.status === "ready" ? "Push. Load. Dump." : completed ? "Route open!" : "Shift over"}</h1><span>{hud.status === "ready" ? "Clear 72% of the route to earn 20 Shells. Lower your shovel with Space, push through the drift, then release on the grass to dump." : completed ? `You opened ${Math.floor(hud.clearedPercent)}% of the route. Your 20-Shell reward is on its way.` : `You opened ${Math.floor(hud.clearedPercent)}% of the route. The next crew will take it from here.`}</span><div className="shoveling-controls" aria-hidden="true"><b>WASD</b><i>line up</i><b>HOLD SPACE</b><i>push</i><b>RELEASE</b><i>dump</i></div><div className="shoveling-mode-actions">{hud.status === "ready" ? <><button type="button" onClick={() => startShift("relaxed")}>Relaxed · No timer</button><button type="button" className="timed" onClick={() => startShift("timed")}>Timed · 1:30</button></> : <button type="button" onClick={() => startShift(hud.mode)}>Shovel again</button>}</div></section> : null}</main>;
}
